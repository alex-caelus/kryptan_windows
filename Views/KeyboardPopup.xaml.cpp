...
Windows::UI::Xaml::Controls::Primitives::Popup^ KeyboardPopup::NewPopup(KeyboardPopup::CloseCallbackFunction closeCallback, Caelus::Utilities::SecureString initialValue, bool isPassword, Platform::String^ hint)
{
    ...
    if (!content->Focus(Windows::UI::Xaml::FocusState::Programmatic))
    {
        throw ref new FailureException("Keyboard not focusable!");
    }
...
void kryptan_windows::KeyboardPopup::doToggleShift()
{
    ...
    {
        Brush^ currentAccentColor = (Brush^)(Application::Current->Resources->Lookup("SystemControlHighlightAccentBrush"));
...
void kryptan_windows::KeyboardPopup::keyboardPopup_LetterKeyDown(Platform::String^ str)
{
    currentText.append(EncodingHandler::extractFromPlatformString(str), 0);
...
void kryptan_windows::KeyboardPopup::keyboardPopup_KeyDown(Platform::Object^ sender, Windows::UI::Xaml::Input::KeyRoutedEventArgs^ e)
{
    switch (e->Key)
    {
        case VirtualKey::Back:
            Back_Click(nullptr, nullptr);
            break;
        case VirtualKey::Enter:
            Done_Click(nullptr, nullptr);
            break;
        ...
        case VirtualKey::Decimal:
            keyboardPopup_LetterKeyDown(",");
            break;
        default:
        {
            auto str = e->OriginalKey.ToString();

            if (str->Length() != 1)
            {
                str = ((int)(e->OriginalKey)).ToString();
            }
            keyboardPopup_LetterKeyDown(str);
            break;
        }
    }
}
