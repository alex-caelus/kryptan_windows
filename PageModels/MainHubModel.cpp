...
Windows::Foundation::Collections::IObservableVector<SecureTextImageSourceDrawLayout^>^ MainHubModel::GetPasswords(Kryptan::Core::PwdLabelVector* labels)
{
    auto list = ref new Platform::Collections::Vector<SecureTextImageSourceDrawLayout^>();

    auto pwds = labels == NULL ? 
        this->pwdFile->GetPasswordList()->All() : 
        this->pwdFile->GetPasswordList()->Filter(*labels);
